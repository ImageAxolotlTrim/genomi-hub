...
这些问题，归根到底都是 DNA 的事。营养、用药、睡眠、运动、遗传性状、某些疾病的风险——背后那套蛋白质、酶、受体、通路怎么搭，很大程度上受它影响。DNA 写不了你的命，但论贴近你自己，没有哪份数据比得过它。
...
3. **把 MCP server 注册给 host agent。**
   ...
   重新加载 host 的 MCP server。要用 URL 喂的话，`llms.txt` 是精简公开地图，`llms-full.txt` 是把公开参考内联进单文件的完整版。
...
一条命令。agent 把 dashboard 能力都对你的基因组扫一遍——变异、ClinVar、药物基因组学、祖源、多基因评分、营养基因组学——把结果做成一个自包含的 dashboard，挂在 localhost 上。浏览器打开就看。
